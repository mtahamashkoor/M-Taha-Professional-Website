...
import { Smartphone, Monitor, ShoppingBag, Palette, Layout, Settings, Sparkles, Check, CheckSquare } from 'lucide-react';
  ...
  return (
    <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 backdrop-blur p-6 md:p-8 shadow-2xl space-y-8" id="showcase">
      ...
      <div className="border-b border-zinc-800 pb-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-center sm:text-left">
        ...
        {/* Simulator Switcher */}
        <div className="flex bg-zinc-950 p-1 rounded-lg self-center sm:self-auto text-xs border border-zinc-800">
          <button
            type="button"
            onClick={() => setActiveTab('ui_builder')}
            className={`px-3 py-1.5 rounded-md font-sans font-bold transition-all ${
      ...
      {activeTab === 'ui_builder' ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="md:col-span-5 bg-zinc-950/80 rounded-xl p-5 border border-zinc-800 space-y-5">
            <h4 className="font-sans text-xs font-extrabold text-zinc-200 uppercase tracking-wider flex items-center gap-1.5 font-mono">
              <Settings className="h-4 w-4 text-emerald-450 text-emerald-400 animate-spin" style={{ animationDuration: '6s' }} />
              <span>Tailwind Component Stylist</span>
            </h4>

            {/* Radius modifier */}
            <div className="space-y-2 text-xs">
              <span className="font-sans font-semibold text-zinc-450 text-zinc-400">Border Radius (Rounded Class)</span>
              ...
              <div className="grid grid-cols-4 gap-1.5">
                {(['indigo', 'emerald', 'amber', 'neutral'] as const).map((col) => (
                  <button
                    ...
                    className={`py-1 rounded text-[10px] font-semibold border transition capitalize ${
                      accentColor === col
                        ? 'bg-emerald-600 text-white border-emerald-500'
                        : 'bg-zinc-900 text-zinc-300 border-zinc-800 hover:bg-zinc-800 hover:text-white'
                    }`}
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="grid grid-cols-2 gap-1.5">
                <button
                  ...
                  className={`py-1.5 rounded text-[10px] font-semibold border transition ${
          ...
          <div className="md:col-span-7 flex items-center justify-center bg-zinc-950 rounded-xl p-4 sm:p-8 border border-zinc-800 min-h-[300px]">
            <div className="w-full max-w-sm space-y-4">
              <span className="text-[10px] font-bold text-zinc-550 text-zinc-500 font-mono tracking-widest block text-center">LIVE RENDERED PREVIEW</span>
              ...
              >
                {/* Element header */}
                <div className="flex items-center justify-between border-b border-zinc-80s border-zinc-800 pb-3 mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold font-mono tracking-wide text-zinc-300">TahaStudio_UI</span>
                  </div>
                  <span className="text-[10px] text-zinc-500 font-medium">100% Client Success</span>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <button
                    type="button"
                    className={`w-full py-2.5 px-4 text-xs font-bold text-white transition-all text-center rounded-lg ${
                      accentColor === 'indigo' ? 'bg-indigo-600 hover:bg-indigo-500' :
                      accentColor === 'emerald' ? 'bg-emerald-600 hover:bg-emerald-500' :
                      accentColor === 'amber' ? 'bg-amber-600 hover:bg-amber-500' : 'bg-zinc-850 bg-zinc-800 hover:bg-zinc-700'
                    }`}
                  >
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="md:col-span-6 bg-zinc-950 border border-zinc-800 rounded-xl p-5 space-y-4">
              ...
              {cartItems.map((item) => (
                <div key={item.id} className="py-2.5 flex justify-between items-center text-xs">
                  <div>
                    ...
                    <span className="text-[10px] text-zinc-500">Digital Deliverable File Code</span>
                  </div>
            ...
            <form onSubmit={handleApplyCoupon} className="space-y-2 pt-3 border-t border-zinc-800">
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="submit"
                  className="px-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-md transition"
                >
                  Verify
                </button>
              </div>
          ...
          <div className="md:col-span-6 bg-[#0e0e11] text-white rounded-xl p-5 flex flex-col justify-between border border-zinc-800">
            ...
            <div className="pt-4 border-t border-zinc-805 border-zinc-800 mt-4 rounded-md">
